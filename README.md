...
选主题配色，调字体、行距和页边距，添加刊头与结尾钩子，在 Obsidian 实时预览后一键复制公众号富文本。

## 功能亮点

- 经典、极简、新锐、杂志四套主题风格，可选择主题配色。
- 调整正文字体、行间距和左右页边距，在 Obsidian 中实时预览。
- 可添加带品牌名和徽章的开头刊头，以及 Markdown 格式的结尾钩子。
- 一键复制当前 Markdown 笔记为公众号富文本，预览与复制结果保持一致。
- 自动将正文外链整理为文末编号引用。
...
[查看 Obsidian 社区插件页面](https://community.obsidian.md/plugins/shuai-wechat-formatter)。
...
- 「预览」：选择右侧分栏或新标签页。
- 「默认排版」：设置正文字体、四套主题风格、配色方案、行间距和页边距。
- 「开头与品牌」：设置品牌名、刊头样式和徽章文字。
- 「结尾内容」：管理需要追加到文章末尾的 Markdown 结尾钩子。
